...
public:
	...
	/// <param name="_data">The bytes to add to the packet.</param>
	void SetBytes(const char* _data)
	{
		Write(_data);
	...
	void WriteLength()
	{
		buffer.insert(intToBytes(buffer.size()).end(), buffer.begin(), buffer.end());
	...
	void Write(const char* _value)
	{
		int count = strlen(_value);
	...
	uint8_t ReadByte(bool _moveReadPos = true)
	{
		if (buffer.size() > readPos)
		{
			// If there are unread bytes
			uint8_t _value = buffer[readPos]; // Get the byte at readPos' position
			if (_moveReadPos)
			{
	...
	/// <param name="_moveReadPos">Whether or not to move the buffer's read position.</param>
	vector<uint8_t> ReadBytes(int _length, bool _moveReadPos = true)
	{
		if (buffer.size() > readPos)
		{
			// If there are unread bytes
			vector<uint8_t> _value;
			_value.assign(buffer.begin() + readPos, buffer.begin() + readPos + _length); // Get the bytes at readPos' position with a range of _length
			if (_moveReadPos)
			{
				// If _moveReadPos is true
	...
	string ReadString(bool _moveReadPos = true)
	{
		try
		{
			...
			while (i < _length) {
				...
				if ((buffer[readPos + i] & 0b11111000) == 0b11110000) {
					char_size = 4;
				}
				else if ((buffer[readPos + i] & 0b11110000) == 0b11100000) {
					char_size = 3;
				}
				else if ((buffer[readPos + i] & 0b11100000) == 0b11000000) {
					char_size = 2;
				}
				else if ((buffer[readPos + i] & 0b10000000) == 0b00000000) {
					char_size = 1;
				}
				else {
					char_size = 1;
				...
				str.append(string(buffer.begin() + readPos + 1, buffer.end() + readPos + char_size + 1));
