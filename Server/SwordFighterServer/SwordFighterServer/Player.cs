...
namespace SwordFighterServer
{
    ...
    public class Player
    {
        ...
        public void AddSchedule(Action action, float delaySecond)
        {
            long delayMs = (long) (delaySecond * 1000f);
        ...
        public void ApplyMovementInput(MoveInput input)
        {
            ...
            position = Utility.ClampPosition(position + input.deltaPos);
        ...
        public void ExecutePlayerSkill(SkillInput skillInput)
        {
            if (currentState == PlayerState.Idle || currentState == PlayerState.Move)
            {
                ...
                var skillData = SkillDatabase.GetSkillData(skillInput.playerSkill);

                AddSchedule(ReturnToIdle, skillData.duration);
